...
const ProductPage = () => {
    ...
    const fetchProducts = async () =>{
      try{
        const response = await fetch(
          `https://6795f9c4bedc5d43a6c3ec8f.mockapi.io/Products`);
          const data = await response.json();
          setProducts(data);
      }catch (error){
        toast.error("Failed to fetch products!", {auttoClose: 2000});
      }
    };
    fetchProducts();
  }, []);
  ...
  const updateProduct = async (updateProduct, id) => {
    try{
      ...
      const response = await fetch(
        `https://6795f9c4bedc5d43a6c3ec8f.mockapi.io/Products/${pro_id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
